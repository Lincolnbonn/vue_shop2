                ...
                <el-table :data="manyTableData" border stripe>
                    ...
                    <el-table-column type="expand">
                        <template slot-scope="scope">
                            ...
                            <!--输入的文本框scope.row.分别绑定到自己的布尔值和value值-->
...
export default{
 ...
 async getParamsData(){
    ...
    const {data:res} = await this.$http.get(`categories/${this.cateId }/attributes`,
      ...
      //三元表达式判断 先对item.attr_vals进行ifelse的判断 如果不为空 直接split 否则返回空白的数组
      res.data.forEach(item=>{
        item.attr_vals= item.attr_vals ?
         item.attr_vals.split(' ') : []
         //为每一行数据单独提供自己的 inputVisible和InputValue
         //控制文本框的显示与隐藏 每一行数据都有自己的布尔值和value值
         item.inputVisible = false
         //文本框中输入的值
         item.inputValue=''
         //每拿到一个item项 就用split做分割 用空格分割原来的字符串
      })//item.attr_vals= 重新赋值 本来是字符串经过for循环后变成数组
      //先做if else的判断 判断数据属于哪个表格 从而挂载到对应的表格中
   ...
   async showEditDialog(attr_id){
        //查询当前参数的信息 attr_id是当前参数的id {params:{attr_sel:是提供一个新的参数
            //const {data:res} = await login(XXX)就是将data重命名为res
   const{data:res} = await this.$http.get(`categories/${this.cateId}/attributes/${attr_id}`,
    {params:{attr_sel: this.activeName}})
      if(res.meta.status !== 200){
        return this.$message.error('获取参数的信息失败！')
      }
      this.editForm = res.data
      this.editDialogVisible = true //必须要有这句 否则对话框不会出现
    },
    //重置修改的表单
    editDialogClosed(){
      this.$refs.editFormRef.resetFields()
    },
  //点击按钮 修改参数信息 const {data:res}结构出data属性 重命名为res
  editParams(){
   this.$refs.editFormRef.validate(async valid=>{
    if(!valid) return
       const {data:res}= await this.$http.put(`categories/${this.cateId}/attributes/
        ${this.editForm.attr_id}`, {attr_name: this.editForm.attr_name,
        attr_sel: this.activeName })
  ...
  //根据id删除对应的参数项 (attr_id)参数id应在参数中获取 因为每个方法都给它传过来了
  ...
  async handleInputConfirm(row){
    ...
    row.inputValue='' //重置为空字符串
    row.inputVisible = false //隐藏文本输入框
    //需要发起请求 保持这次操作 attr_vals服务器存储的是字符串 客户端存储的是数组
   ...
   const {data:res} = await this.$http.put(`categories/${this.cateId}/attributes/${row.attr_id}`,
    {
        ...
        attr_vals: row.attr_vals.join(' ')//将数组用join做拼接 里面每一项用空格来做拼接
    })
     if(res.meta.status !== 200){
        return this.$message.error('修改参数的失败！')
     }
